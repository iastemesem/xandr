...
data class HostAPIUserId (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): HostAPIUserId {
      val source = pigeonVar_list[0] as HostAPIUserIdSource
...
private open class XandrPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          HostAPIUserIdSource.ofRaw(it.toInt())
        }
      }
      130.toByte() -> {
...
interface XandrHostApi {
  ...
  fun resetController(callback: (Result<Unit>) -> Unit)
  ...
  fun setAutoRefreshInterval(autoRefreshIntervalInSeconds: Long, inventoryCode: String?, placementID: String?, callback: (Result<Boolean>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: XandrHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val memberIdArg = args[0] as Long
            val publisherIdArg = args[1] as Long?
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.xandr_android.XandrHostApi.resetController$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            api.resetController{ result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val autoDismissDelayArg = args[0] as Long?
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.xandr_android.XandrHostApi.setAutoRefreshInterval$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val autoRefreshIntervalInSecondsArg = args[0] as Long
            val inventoryCodeArg = args[1] as String?
            val placementIDArg = args[2] as String?
            api.setAutoRefreshInterval(autoRefreshIntervalInSecondsArg, inventoryCodeArg, placementIDArg) { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
